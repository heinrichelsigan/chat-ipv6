...
namespace EU.CqrXs.Srv.Svc.Swashbuckle.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class ChatPollAllController : CqrSrvControllerBase
    {

        private readonly ILogger<ChatPollAllController> _logger;

        public ChatPollAllController(ILogger<ChatPollAllController> logger)
        {
            _logger = logger;
        }

        [HttpGet(Name = "ChatPollAll")]
        public string Get(string cryptMsg)
        {
            Area23Log.LogStatic("ChatPollAll(string cryptMsg) called.  cryptMsg.Length = " + cryptMsg.Length + ".\n");
            InitMethod();
            Area23Log.LogOriginMsg("CqrService", $"ChatPushPoll(string cryptMsg) called.\n");
            InitMethod();
